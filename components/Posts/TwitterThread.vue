...
<template>
  <div
    v-if="post"
    class="bg-white dark:bg-gray-800 dark:border-gray-800 p-4 rounded-xl border relative"
    :class="{
      'border-emerald-400 border-dashed	':
        isSelectable &&
        selectedCreationPostsTwitter &&
        selectedCreationPostsTwitter.uuid === post?.uuid,
      'border-gray-100':
        (isSelectable &&
          selectedCreationPostsTwitter &&
          selectedCreationPostsTwitter.uuid !== post?.uuid) ||
  ...
  >
    <UButton
      ...
      square
      class="absolute icon-position border border-gray-100"
    />
    ...
    <div class="flex">
      <div class="w-11 pt-2 justify-center flex items-center">
        <div class="bg-gray-300 w-[1px] h-full"></div>
      </div>
      <div class="flex-1 pl-4">
        <p class="text-black dark:text-white block text-xl leading-snug mt-3">
          {{ post?.body }}
        </p>
        <PostsTwitterPictures :post="post" />
        <div class="text-gray-500 dark:text-gray-400 flex mt-3">
          ...
          <div class="flex items-center mr-6">
            ...
            <span class="ml-3">115 people are Tweeting about this</span>
          </div>
        </div>
      </div>
    </div>
